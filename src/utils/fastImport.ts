 ...
 *    - Cột `Giá trị`, `Chuyên trang`, `Ghi chú chi tiết`, và `Tên sản phẩm` luôn được bỏ trống khi xuất Excel cho Hợp đồng mới (status = 2) theo yêu cầu nghiệp vụ.
...
export function buildFastImportRows(
  ...
  return rows.map((row, index) => ({
    ...
    'Giá trị': (options.status === 1 || options.status === 2 || sttMode === 'sequential') ? '' : valueOrZero(row.giaTri ?? row.thanhTienSauCk),
    ...
    'Chuyên trang': options.status === 2 ? '' : valueOrEmpty(row.chuyenTrangImport ?? row.chuyenTrang),
    'Ghi chú chi tiết': options.status === 2 ? '' : valueOrEmpty(row.ghiChuChiTiet ?? row.ghiChu),
    ' ': '',
